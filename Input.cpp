...
int Input::GetInteger(Output* pO) const
{
	...
	string strtoint;
	strtoint = GetSrting(pO);
	int myint = stoi(strtoint);


	// Note: stoi(s) converts string s into its equivalent integer (for example, "55" is converted to 55)

	return myint; // this line should be changed with your implementation
}
...
CellPosition Input::GetCellClicked() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN)
	{
		if (y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			/*bool HCell = */cellPos.SetHCell(x/UI.CellWidth);
			/*bool Vcell =*/ cellPos.SetVCell((y - UI.ToolBarHeight)/UI.CellHeight);
