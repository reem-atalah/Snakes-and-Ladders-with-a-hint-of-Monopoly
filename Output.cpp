...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_EXITD] = "images\\Menu_Exit.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\Menu_Input_Dice.jpg";
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int HorizontalLines = y1 - UI.LadderYOffset + UI.CellHeight;
	do {
		HorizontalLines -= UI.CellHeight;
		pWind->DrawLine(x12, HorizontalLines, x34, HorizontalLines, FRAME);
	} while (HorizontalLines > (y2 - UI.LadderYOffset + UI.CellHeight));
}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int X_Coor_Head[4] = { x12 , x12+ xChange, x12, x12 - xChange };
	int Y_Coor_Head[4] = { y1 + yChange,  y1 , y1 - yChange, y1 };


	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	pWind->DrawPolygon(X_Coor_Head, Y_Coor_Head, 4, FILLED);
