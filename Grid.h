...
public:
	...
	virtual void SaveAll(ofstream& ,Type);
	//Ask the TAs about those getters
	...
	//Added this , make sure that it doesn't violate class responsibilities
	Card* GetNextCard(const CellPosition& position);   // Gets a Pointer to the first card after the passed "position"
	void setCurrentPlayerNumber(int c);   //A setter for the curent player number
	int getCurrentPlayerNumber();         //A getter for the curent player number
